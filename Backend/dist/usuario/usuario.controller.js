...
const em = orm.em;
async function sanitizeUsuario(req, res, next) {
...
async function loginCtrl(req, res) {
    ...
    try {
        ...
        if (usuario && isValid) {
            ...
            const token = jwt.sign({ idUsuario: idUsuario, email: email, perfilImage: usuario.perfilImage, nombre: usuario.nombre, tipoUsuario: usuario.tipoUsuario }, SECRET_JWT_KEY, { expiresIn: '1h' });
            ...
            res.status(200).json({ message: 'Login successful', data: { idUsuario: usuario.idUsuario, email: usuario.email, perfilImage: usuario.perfilImage }, token: token });
            return;
        }
        else {
...
async function authenticate(usuario, res) {
...
async function authMiddleware(req, res, next) {
    ...
    req.usuario; // Inicializa el usuario como null
    if (!token) {
